...
function addList(...someNum) {
  return someNum.reduce((acc, val) => acc + val, 0)
}
...
function computeRemainder(num1, num2) {
  ...
  return Number(((num1/num2 - Math.floor(num1/num2)) * num2).toFixed(8))
}

console.log(computeRemainder(5, 0))
...
function charCount(strStr) {
  const newArr = strStr.split('')
  let cntArr = []
  for (let value of newArr) {
    let count = 0
    for (let othValue of newArr) {
      if (value === othValue) {
        count += 1
      }
    }
  ...
  return newArr.reduce((acc, val, ind) => {
...
formatWithPadding = (integr, padChar, minLength) => {
  const integrLength = integr.toString().length
  for (let i = integrLength; i < minLength; i++) integr = padChar + integr
  return `${integr}`
}
...
function isPalindrome(str) {
  return str.toUpperCase().split(' ').join('') === str.toUpperCase().split(' ').join('').split('').reverse().join('')
}
...
// need to get number length of a and b and compare at each PointerEvent
// whenever mismatch need to count 1 
// if counting for one and other dont exist return nan 
function hammingDistance(str1, str2) {
  return [...str1 + str2].reduce((acc, el, ind) => {
    if (ind < str1.length) {
      if (el === str2[ind]) {
        el = 0
      } else {
    ...
    if (ind < str2.length * 2 && ind >= str2.length) {
      if (el === str1[ind - str1.length]) {
        el = 0
      } else {
        el = 1
      }
    }
    return Number(acc + el)
  }, 0) / 2
}


// console.log(hammingDistance('a**0edss', 'appdlssa'))
...
function mumble(str) {
  return str.split('').reduce((a, letter, index, strArr) => {
    for (i = 0; i <= index; i++) {
      a += letter
    }
...
function mergeObjects(...objs) {
  return [...objs].reduce((a, obj, idx) => {
    if (idx > 0) {
      for (let key in obj) a[`${key}`] = obj[key]
    }
    return a
  }, [...objs][0])
}
...
function findHighestPriced(itemObjs) {
   return [...itemObjs].sort((a, b) => b.price - a.price)[0]
}
...
function mapArray(arr, cb) {
  return arr.reduce((ret, el, index) => {
    ret.push(cb(el, index))
    return ret
  }, [])
}
...
function flatten(arrArrs) {
  const flatArr = []
  let toPush
  for (let arr of [...arrArrs]) {
    if (Array.isArray(arr)) {
      flatArr.push(...flatten(arr))
      continue
    } else {
      flatArr.push(arr)
    }  
...
function isPrime(num) {
  let bool = num === parseInt(num) && num > 1
  if (num !== 2) for (let i = 2; i < num; i++) if (num % i === 0) bool = false
  return bool
}
// console.log('prime', isPrime(9))
...
function primeFactors(num) {
  let arr = []
  let arr2 = []
  let IntermediateVal = num
  if (num === parseInt(num) && num > 1) for (let i = 2; i <= num; i++) if (num % i === 0) arr.push(i)
  for (let value of arr) for (let j = 2; j < value; j++) if (value % j === 0) arr.splice(arr.indexOf(value), 1)
  for (let value of arr) while (IntermediateVal % value === 0) {
    arr2.push(value)
...
function intersection(arr1, arr2) {
  let playArr = arr1.length < arr2.length ? [...arr1] : [...arr2]
  let restArr = arr1.length < arr2.length ? [...arr2] : [...arr1]
  return playArr.reduce((acc, el) => {
    for (let value of restArr) if (value === el) {
      acc.push(el)
      break
    }
      return acc
...
function balancedBrackets(bracesStr) {
  let bool = bracesStr.length % 2 === 0
  let matches = {
    '(': ')',
    '{': '}',
    '[': ']'
  }
  for (let i = 0; i < bracesStr.length / 2; i++) {
      if (matches[`${bracesStr[i]}`] !== bracesStr[bracesStr.length - i - 1]) {
        let subStrTest
        for (let j = i + 1; j < bracesStr.length - i; j++) if (matches[`${bracesStr[i]}`] === bracesStr[j]) {
            subStrTest = bracesStr.substring(i, j + i - 1)
        }
        if (!subStrTest) {
          bool = false
        } else if (!balancedBrackets(subStrTest)) {
          bool = false
        }
        break
      }
...
function isWinningTicket(arr) {
  return arr.reduce((a, subArr) => {
    a.push(subArr[0].includes(String.fromCharCode(subArr[1])))
    return a
  }, []).every(e => e)
}
...
function getNumForIP(ip) {
  return ip.split('.').reverse().reduce((acc, val, index) => acc += (Math.pow(256, index)) * val, 0)
}
...
function toCamelCase(str) {
  ...
  indices = [...str.matchAll(re)].map(match => match.index)
  let subStr = str
  let count = 0
  for (let i of indices) {
    const nextLetter = subStr[i + 1 - count].toUpperCase()
    subStr = subStr.replace((subStr[i - count] + subStr[i + 1 - count]), nextLetter)
    count += 1
  }
  return subStr
}
...
function countTheBits(num) {
  return num.toString(2).split('').filter(val => parseInt(val) === 1).length
}
...
function gridTrip(start, moves) {
  ...
  let y = start[1]
  const re = /[UDRL]/g
  const indices = [...moves.matchAll(re)].map(match => match.index)
  const singleMoves = []
  for (let i of indices) singleMoves.push(moves.slice(i , indices[indices.indexOf(i) + 1]))
  console.log(singleMoves)
  for (let move of singleMoves) {
    if (move[0] === 'U') x += parseInt(move.slice(1))
    if (move[0] === 'D') x -= parseInt(move.slice(1))
    if (move[0] === 'R') y += parseInt(move.slice(1))
    if (move[0] === 'L') y -= parseInt(move.slice(1))
  }
  return [x, y]
}
...
function addChecker(arr, num) {
  ...
  for (let val of arr) {
    bool = false
    for (let compareVal of arr.slice(arr.indexOf(val) + 1)) {
      if (val + compareVal === num) {
        bool = true
        break
      }
    }
...
function totalTaskTime(tasks, threads) {
  ...
  let currThreads
  let tasksLeft
  while (tasks.length > threads) {
    currThreads = tasks.slice(0, threads);
    least = Math.min(...currThreads)
    totalTime += least
    currThreads = currThreads.map(thread => thread - least).filter(t => t)
    tasks = currThreads.concat(tasks);
  }
  if (tasks.length) tasksLeft = Math.max(...tasks)
  else tasksLeft = 0
  return totalTime + tasksLeft
}
